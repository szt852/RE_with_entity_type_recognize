...
sentence_list的数据量: 10626
删除不可能存在的关系后sentence_list的数据量: 10626
去重后关系后sentence_list的数据量: 10613
文件 ../datasets/CMeIE_org/CMeIE_train.jsonl 拥有的句子数量： 14339
sentence_list的数据量: 54273
删除不可能存在的关系后sentence_list的数据量: 54273
去重后关系后sentence_list的数据量: 54210
所有文件拥有的句子数量： 54210
...
def find_pos(json_content,sentence_list,json_path):
    '''创建实体下标'''
    ...
    # 删除不可能存在的关系
    print("sentence_list的数据量:", len(sentence_list))
    jiangyou_list = []
    for m in tqdm(range(len(sentence_list))):
        if (sentence_list[m]['relation'] in relation_list):
            jiangyou_list.append(sentence_list[m])
    sentence_list = jiangyou_list
    print("删除不可能存在的关系后sentence_list的数据量:", len(sentence_list))

    # 字典列表去重
    unique_list = []
    temp_no_re_list = sorted(sentence_list, key=lambda x: x['sentence'])
    for n in tqdm(range(len(temp_no_re_list))):
        if temp_no_re_list[n] not in unique_list:
            unique_list.append(temp_no_re_list[n])
    sentence_list = unique_list
    print("去重后关系后sentence_list的数据量:", len(sentence_list))
...
sentence_list=find_pos(read_json(dev_json_path),sentence_list,dev_json_path)
train_json_path='../datasets/CMeIE_org/CMeIE_train.jsonl'
sentence_list=find_pos(read_json(train_json_path),sentence_list,train_json_path)
...
two_e_sentence_list = fix_pos(sentence_list)

CMeIE_ok_path = '../datasets/CMeIE/CMeIE_ok.txt'
with open(CMeIE_ok_path, 'w+', encoding='utf-8') as f:
