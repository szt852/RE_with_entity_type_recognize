...
def change_temp1_2_temp2(path):
    ...
    print("文件",path,"拥有的句子数量：",len(json_data))
    # print(json_content)
    for i in tqdm(range(len(json_data))):
        global false,true
        false = False
        true = True
        i_dict = eval(json_data[i])
        # print(i_dict['relation'])
        if (i_dict['relation'] not in re_set_list):
            re_set_list.append(i_dict['relation'])
...
# no_re_list = []
re_set_list = []
dev_json_path='../diakg_temp1/dev.txt'
test_json_path='../diakg_temp1/test.txt'
train_json_path='../diakg_temp1/train.txt'

for path_i in [dev_json_path,test_json_path,train_json_path]:
    # tmp_sentence_list = []
    # tmp_no_re_list = []
    change_temp1_2_temp2(path_i)
    print(path_i)
    print("re_set_list(",len(re_set_list),"): ",re_set_list)
    ...
    #
    # tmp_dataset = path_i.split('/')[2]
    # # two_e_sentence_no_re_list = fix_pos(tmp_sentence_list + tmp_no_re_list)
    # re_ok_path = '../../datasets/diakg_org_split/' + tmp_dataset
    # with open(re_ok_path, 'w+', encoding='utf-8') as f:
